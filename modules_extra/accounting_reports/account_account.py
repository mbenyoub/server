...
#                if can_compute:
    ...
    def _prepare_compute(self, cr, uid, ids, field_names, arg=None, context=None):
        ...
        if context.has_key('date_from'):
            ...
            end_period_id = fiscalperiod_obj.browse(cr, uid, [ctx['period_from']], context=ctx)[0]
            ctx['period_to'] = fiscalperiod_obj.search(cr, uid, [('date_start','<',end_period_id.date_start)], order='date_start DESC', limit=1)[0]
            
            period_company_id = fiscalperiod_obj.browse(cr, uid, ctx['period_from'], context=ctx).company_id.id
            first_period = fiscalperiod_obj.search(cr, uid, [('company_id', '=', period_company_id)], order='date_start', limit=1)[0]
            ctx['period_from'] = first_period
