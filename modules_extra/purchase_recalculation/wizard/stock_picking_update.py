...
class stock_picking_update_line(osv.TransientModel):
    ...
    _columns = {
        ...
        'quantity_new'  : fields.float("New Quantity", digits_compute=dp.get_precision('Product Unit of Measure'), required=True),
        'price_unit'    : fields.float("Price Unit", digits_compute=dp.get_precision('Product Price')),
        'price_unit_new': fields.float("New Price Unit", digits_compute=dp.get_precision('Product Price'), required=True),
        'move_id'       : fields.many2one('stock.move', "Move", ondelete='CASCADE'),
        'wizard_id'     : fields.many2one('stock.picking.update', string="Wizard", ondelete='CASCADE'),
    }

    def fields_view_get(self, cr, uid, view_id=None, view_type=False, context=None, toolbar=False, submenu=False):
        res = super(stock_picking_update_line, self).fields_view_get(cr, uid, view_id=view_id, view_type=view_type, context=context, toolbar=toolbar, submenu=submenu)
