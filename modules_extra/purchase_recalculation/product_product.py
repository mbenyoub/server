...
class product_product(osv.Model):
    ...
    def _get_stock_and_cost_inventory(self, cr, uid, ids, dates, context):
        if dates is None:
            dates = {}
        _stock = 0.0
        _cost = 0.0
        ...
        _inventory_line_ids = self.pool.get('stock.inventory.line').search(cr, uid, [('product_id','=',ids[0])])
        ...
        for _inventory_line in _inventory_lines:
            if _inventory_line.inventory_id.date_done >= _date_from \
               and _inventory_line.inventory_id.date_done <= _date_to \
               and _inventory_line.inventory_id.state == 'done':
                for _move_line in _inventory_line.inventory_id.move_ids:
                    _stock = _stock + _move_line.product_qty
                    _acc_move_line_id = self.pool.get('account.move.line').search(cr, uid, [('stock_move_id','=',_move_line.id)], limit=1)
    ...
    def _get_stock_and_cost(self, cr, uid, ids, dates, context):
        if dates is None:
            dates = {}
        _stock = 0.0
        _cost = 0.0
        ...
        if _date_from and _date_to:
            ...
            _last_out_move_tups = cr.fetchall()
            _last_out_move_id = []
            for _tup in _last_out_move_tups:
                _last_out_move_id.append(_tup[0])
            ...
            if len(_last_out_move_id) == 1:
                ...
                _cost = _last_out_move.price_unit
    ...
    def _get_purchase_amount_and_cost(self, cr, uid, ids, dates, digits, context):
        ...
        return [_stock, round(_cost, digits['price'])]
