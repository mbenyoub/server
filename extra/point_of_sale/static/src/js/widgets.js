    ...
    module.PaypadButtonWidget = module.PosBaseWidget.extend({
        ...
        renderElement: function() {
            ...
            this.$el.click(function(){
                ...
                // Valida que tenga un cliente para aplicar credito
                console.log('****VALIDANDO SI HAY CLIENTE PARA APLICAR CREDITO****');
                if (self.cashRegister.get('journal').self_apply_credit) {
                    var client  = self.pos.get('selectedOrder').get_client();
                    if (!client) {
                        alert('No puede asignar credito si no hay un cliente seleccionado');
                        return;
                    }
                }
        ...
        renderElement: function() {
            ...
            $("a", this.$el).click(function(e){
                if(self.click_product_action){
                    // Validando que antes de seleccionar un producto exista un cliente seleccionado
                    var client_1 = self.pos.get('selectedOrder').get_client();
                    //if (!client_1) {
