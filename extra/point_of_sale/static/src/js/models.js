function openerp_pos_models(instance, module){ //module is instance.point_of_sale
        ...
        load_server_data: function(){
            ...
            var loaded = self.fetch('res.users',['name','company_id'],[['id','=',this.session.uid]]) 
                .then(function(users){
                    ...
                    //ADD AKKADIAN - Agrega validacion para solicitar descuento
                    return self.fetch(
                        'pos.config',
                        ['name','journal_ids','shop_id','journal_id',
                         'iface_self_checkout', 'iface_led', 'iface_cashdrawer','iface_apply_discount',
                         'iface_payment_terminal', 'iface_electronic_scale', 'iface_barscan', 'iface_vkeyboard',
                         'iface_print_via_proxy','iface_cashdrawer','state','sequence_id','session_ids'],
                        [['id','=', self.get('pos_session').config_id[0]]]
                    );
                // ADD AKKADIAN - Validacion para solicitar descuento
                    ...
                    self.iface_apply_discount      =  !!pos_config.iface_apply_discount;
    ...
    module.Paymentline = Backbone.Model.extend({
        ...
        export_as_JSON: function(){
            var credit = this.cashregister.get('journal').self_apply_credit
            //alert ('Exportar a JSON ' + this.cashregister.get('journal_id') + ' - ' + this.cashregister.get('journal').name + ' - ' + credit)
            return {
    ...
    module.Order = Backbone.Model.extend({
        ...
        addPaymentLine: function(cashRegister) {
            ...
            if(cashRegister.get('journal').type == 'cash'){
                ...
                newPaymentline.set_amount(this.getTotalTaxIncluded());
            }
        ...
        get_user: function(){
            return this.pos.get('cashier') ? this.pos.get('cashier') : this.pos.get('user');
        },
        ...
        export_for_printing: function(){
            ...
            //ADD AKKADIAN - Corrige el retorno del cliente
            var client_name = 'Venta de mostrador'
            if (client) {
        ...
        exportAsJSON: function() {
            ...
            (this.get('paymentLines')).each(_.bind( function(item) {
                //alert ('pago ' + item.export_as_JSON().journal_id + ' - aplica credito: ' + item.export_as_JSON().credit, item.export_as_JSON().amount)
                if (item.export_as_JSON().credit) {
                    credit = credit + item.export_as_JSON().amount
                }
                
                return paymentLines.push([0, 0, item.export_as_JSON()]);
            }, this));
            
            alert('Export original')
            
            //ADD AKKADIAN - Correccion de retorno ID de cliente
            var client_id = undefined;
            var client_credit = 0;
            if (this.pos.get('client'))
            {
                client_id = this.pos.get('client').id;
                client_credit = Number(Math.round(this.pos.get('client').credit_available+'e'+2)+'e-'+2);
            }
            //alert ('Credito: ' + Math.round(credit) + ' - Credito cliente: ' + Math.round(client_credit) + 'Monto total: ' + Math.round(this.getTotalTaxIncluded()))
            
            if (client_credit < Math.round(credit)) {
                alert('El credito disponible del cliente es insuficiente para completar la transaccion (Disponible: $ ' + credit + ').')
                return false
            }
            else if (Math.round(credit) > Math.round(this.getTotalTaxIncluded())) {
                alert('El credito no debe ser mayor al monto a pagar sobre el pedido')
                return false
            }
            else{
                return {
