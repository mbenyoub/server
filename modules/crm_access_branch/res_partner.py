...
class res_partner(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        """
            Actualiza la sucursal para todos los hijos relacionados al contacto
        """
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            # Si es una empresa actualiza los hijos con la misma sucursal
            if partner.is_company:
                # Ejecuta consulta para actualizar registros
                cr.execute('update res_partner set branch_id=%s where parent_id=%s', (partner.branch_id.id, partner.id))
        return True
