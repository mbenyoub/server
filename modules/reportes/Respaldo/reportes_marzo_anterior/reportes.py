...
#__________________________________________________Variable de formato de consulta____________________________________________________________#
        ...
        if reporte.reporte == 'bal':
            ...
            com[0] = {0:report + " " + self._get_mes(cr, uid, reporte.mes, context) + ' ' + reporte.periodo.name, 3:company}
        else:    
            ...
            for num in range(len(est)):
                ...
                if num == 0:
                    ...
                    for index, col in enumerate(fila):
                        comp = com[0]
                        if index == 0:
                            row.write(index, comp[col], xlwt.easyxf("font: name Arial, height 280, color blue, bold on; align: wrap on, vert centre, horiz center"))#pattern: pattern solid, fore_color white;                             
                            row.height_mismatch = True
                            row.height = 30 * 30
                        elif index == 1:
                            row.write(index, comp[col], xlwt.easyxf("font: name Arial, height 280, color blue, bold on; align: wrap on, vert centre, horiz center"))
                            row.height_mismatch = True
                            row.height = 30 * 30
                else:
