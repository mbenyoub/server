...
openerp.canis = function (instance) {
    ...
    */
    module.CustomerOrderWidget = module.PosBaseWidget.extend({
        ...
        get_credit: function(){
            customer = this.pos.get('selectedOrder').get_client();
            if(customer){
                var credit = customer.credit_available 
                return '(Credito: $' + credit.toString() + ')';
            }else{
                return "";
            }
        },
    ...
    module.CustomerWidget = module.PosBaseWidget.extend({
        ...
        renderElement: function() {
            ...
            $("a", this.$el).click(function(e){
                ...
                //alert('Cliente select' + self.model.toJSON().name)
                //alert('Credito disponible' + self.model.toJSON().credit_available)
    ...
    module.PosModel = module.PosModel.extend({
        load_customers_data: function(){
            ...
            var loaded = self.fetch(
                    ...
                    'property_product_pricelist','vat','debit_limit','credit_limit','credit_available',
                    'email','website','fax','phone','mobile',],
                    [['customer', '=', true],'|',['branch_id.user_ids','in',[user]],['branch_id','=',false]])
    ...
    module.Order = module.Order.extend({
        ...
        exportAsJSON: function() {
            ...
            if (this.pos.get('client'))
            {
                ...
                client_credit = this.pos.get('client').credit_available
            }
            //alert ('Credito: ' + Math.round(credit) + ' - Credito cliente: ' + Math.round(client_credit) + 'Monto total: ' + Math.round(this.getTotalTaxIncluded()))
            
            // Valida que haya un cliente seleccionado para finalizar el proceso
            if (client_id == undefined) {
                alert('No hay un cliente seleccionado sobre el pedido')
                return false
            }
            
            if (Math.round(client_credit) < Math.round(credit)) {
