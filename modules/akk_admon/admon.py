...
# ---------------------------------------------------------
# Funciones para validacion de codigo de producto EAN13
# ---------------------------------------------------------
...
class admon_database(osv.osv):
    _name = "admon.database"
    ...
    def _get_num_users(self, cr, uid, ids, name, args, context=None):
        """
            Obtiene los usuarios activos en el sistema
        """
        result = {}
        for obj in self.browse(cr, uid, ids, context=context):
            user_ids = self.pool.get('admon.database.user').search(cr, uid, [('state','in',['active']),('superuser','=',False),('database_id','=',obj.id)], context=context)
            result[obj.id] = len(user_ids)
        return result
    ...
    _columns = {
        ...
        'info_id': fields.many2one('admon.database.info', 'Informacion de Registro'),
        'partner_id': fields.many2one('res.partner', 'Cliente', select=True, ondelete='cascade', required=True),
        'max_user': fields.integer('Usuarios disponibles'),
        ...
        'date_ini': fields.date('Fecha creacion'),
        'date_end': fields.date('Fecha vencimiento'),
        'user_ids': fields.one2many('admon.database.user', 'database_id', 'Usuarios'),
        'module_ids': fields.one2many('admon.database.module.relation', 'database_id', 'Modulos instalados'),
        'active_users': fields.function(_get_num_users, type="integer", string="usuarios activos")
    }
    
    _defaults = {
        'state': 'pending',
        'date_ini': fields.datetime.now,
        'max_user': 1
    }
    
    #def action_update_modules(self, cr, uid, ids, context=None):
    #    """
    #        Actualiza los modulos especificados en la base de datos
    #    """
    #    update_ids = []
    ...
    #    for db in self.browse(cr, uid, ids, context=context):
    #        db_name = db.code
    #        # Recorre las bases de datos a aplicar
    #        for rel in db.module_ids:
    #            mod_name = rel.module_id.name
    #            # Actualiza el registro
    #            mod_obj.update_module(db_name, mod_name)
    ...
    def get_config_password(self, cr, uid, context=None):
        """
            Obtiene el pasword asignado para el administrador
        """
        config_id = False
        res = 'admin'
        cr.execute(
    ...
    def get_config_database(self, cr, uid, context=None):
        """
            Obtiene la configuracion de la base de datos
        """
        config_id = False
        ...
        if config_id:
            config = self.pool.get('admon.config.settings').browse(cr, uid, config_id, context=context)
            res['database'] = config.db_list
        return res
    
    def action_create_database(self, cr, uid, ids, context=None):
        """
            Crear la base de datos
        """
        # Crea un nuevo registro
        db = ws()
        data = self.get_config_database(cr, uid, context=context)
        db_original_name = data['database']
        mod_obj = self.pool.get('admon.database.module')
        ...
        # Recorre los registros
        for database in self.browse(cr, uid, ids, context=context):
            db_name = database.code
            # Valida que no exista la base de datos
            db_exist = db.exp_db_exist(db_name)
            if db_exist == True:
                raise osv.except_osv(_('Error!'),_("Ya existe una base de datos registrada con el nombre '%s'")%(db_name,))
            
            try:
                # Crea la base de datos
                db.exp_duplicate_database(db_original_name, db_name)
            except:
                # Elimina las conexiones activas
                cr.execute("""
                 SELECT pg_terminate_backend(procpid) FROM pg_stat_activity
                 WHERE datname='%s' AND procpid<>pg_backend_pid()"""%(db_original_name,))
                # Crea la base de datos
                db.exp_duplicate_database(db_original_name, db_name)
                
            # Revisa si hay modulos por default para agregar
            mod_ids = mod_obj.search(cr, uid, [('admon_default','=',True)], context=context)
            # Recorre los modulos y agrega la relacion con la base de datos
            for mod_id in mod_ids:
                mod_rel_obj.create(cr, uid, {'database_id': database.id, 'module_id': mod_id}, context=context)
            
            # Obtiene el profile de administrador
            model = 'akk_admon'
            xml_id = 'user_profile_3'
            profile_id = data_obj.get_object(cr, 1, model, xml_id).id
            password = self.get_config_password(cr, uid, context=context)
            
            # Agrega al usuario administrador en la lista
...
class admon_database_info(osv.osv):
    ...
    _columns = {
        ...
        'company_name': fields.char('Nombre Comercial', size=264),
        ...
        try:
            ...
            for reg in import_data:
                ...
                partner_data = {
                    ...
                    'street': reg[6],
                    'street2': reg[7],
                    'l10n_mx_street4': reg[8],
                    'l10n_mx_street3': reg[9],
                    'city': reg[10],
            ...
            for reg in import_data:
                ...
                if reg[2]: 
                    ...
                    if product_data['categ_id']:
                        categ = categ_obj.browse(cr, uid, product_data['categ_id'])
                        if categ.type_product:
                            product_data['type'] = categ.type_product
                        if product_data.get('uom_id',False) == False:
                            product_data['uom_id'] = categ.uom_id.id or False
                            if categ.uom_po_id:
                                product_data['uom_po_id'] = categ.uom_po_id.id or False
    ...
    def update_info_company(self, db_name, vals, reg):
        ...
        try:
            ...
            if vals.get('company_name',False):
                company_name = vals.get('company_name','')
            
            # Actualiza la informacion de la compañia
            com_obj.write(cr, uid, [company.id], {'company_registry': vals.get('rfc',''), 'vat': vals.get('vat',''), 'name': company_name}, context=context)
            
            print "*************** rfc ******************** ", vals.get('rfc','')
    ...
    def action_update_pac_sf(self, cr, uid, ids, context=None):
        ...
        for info in self.browse(cr, uid, ids, context=context):
            ...
            # Actualiza la informacion del usuario y password
            vals.update({
                'user': info.sf_email,
                'password': info.sf_password,
                'active': True
            })
