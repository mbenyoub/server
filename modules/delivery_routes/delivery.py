...
class delivery_route_line(osv.osv):
    ...
    def _get_id(self, cr, uid, ids, field_name, args, context={}):
        """
            Metodo para obtener el id de la entrega y utilizarlo para crear el vale de entrega
        """
        result={}
        mostrar = True
        for id in ids: 
            cr.execute("""
                         UPDATE delivery_route_line SET r_line_id = %s
                        """,(id,))
    ...
    def button_print(self, cr, uid, ids, context=None):
        """
            Metodo para generar el reporte del vale de entrada desde el wizard de la entrega
        """
        datas = {}
        if context is None:
            context = {}
        ...
        datas = {
            'ids': ids,
            'model': 'delivery.route.line', # Clase de donde se obtendra el reporte
            'form': data
        }
        print "*****DATAS*****: ", datas
        return {
            'type': 'ir.actions.report.xml',
            'report_name': 'imprimir.entrega', # Nombre del reporte en el xml donde se crea este
            'datas': datas
        }
    ...
    _columns = {
        ...
        # Informacion chofer
        'driver_id': fields.function(_get_driver, type='many2one', relation="delivery.driver",
            store=_store_drivers, string='Chofer'),
        'r_line': fields.function(_get_id, type='boolean', string='idb', method=True,store=False),
        'r_line_id': fields.integer('id'),
        ...
        'log_line_ids': fields.one2many('delivery.route.line.log', 'route_line_id',
            'Registro Entregas Ruta', readonly=True),
        # 'paid_ids': fields.one2many('delivery.route.paid.log', 'route_line_id', 'Pagos')
        'paid_log_ids': fields.one2many('delivery.route.paid.log', 'r_line_id', 'Pagos realizados')
