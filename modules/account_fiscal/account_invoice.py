...
class account_invoice(osv.Model):
    ...
    def unlink(self, cr, uid, ids, context=None):
        """
            Modifica el campo de internal_number sobre la factura de proveedor para que deje borrarlas
        """
        if context is None:
            context = {}
        # Obtiene los campos a validar sobre las facturas
        invoices = self.read(cr, uid, ids, ['state','type','internal_number'], context=context)
        supplier_ids = []

        # Recorre las facturas
        for t in invoices:
            # Si es una factura de proveedor en borrador y tiene el numero interno registrado toma la referencia
            if t['state'] in ('draft') and t['internal_number'] and t['type'] == 'in_invoice':
                supplier_ids.append(t['id'])

        # Quita el numero interno de la factura de proveedor
        if supplier_ids:
            self.write(cr, uid, supplier_ids, {'internal_number': False}, context=context)
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        """
            ...
            Relaciona la factura con la venta o compra de origen
        """
        if default is None:
            default = {}
        default.update({
        ...
        print "**************** Vaida copy ref **************** ", 'ref' in default

        # Obtiene la factura duplicada
        invoice = self.browse(cr, uid, id, context=context)

        print "********* referencia sobre factura ********* ", invoice.ref
        # Valida si hay una referencia
        if invoice.ref:
            model = invoice.ref._name
            print "************ model ************ ", model
            # Valida si es una compra o venta
            if model == 'sale.order' or model == 'purchase.order':
                inv_ids = [res]
                # Actualiza la lista de facturas registradas
                for inv in invoice.ref.invoice_ids:
                    inv_ids.append(inv.id)
                print "************* invoice_ids ************ ", inv_ids
                # Acualiza la lista de facturas relacionadas
                self.pool.get(model).write(cr, uid, [invoice.ref.id], {'invoice_ids': [[6, False, inv_ids]]})
...
class account_invoice_line(osv.Model):
    ...
    _columns = {
        ...
        'invoice_expense': fields.related('invoice_id', 'invoice_expense', type='boolean', String="Factura Gasto", store=True, readonly=True)
    }
