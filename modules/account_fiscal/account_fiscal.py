...
class account_fiscal_code(osv.Model):
    ...
    def get_value_code_isr(self, cr, uid, period_id, code_id, code_amount, value_isr='amount', apply_year=False, context=None):
        """
            Retorna el valor obtenido de la tabla de isr a travez del codigo fiscal como referencia para el limite inferior y limite superior
        """
        print "****************** periodo *********** ", period_id
        print "****************** codigo *********** ", code_id
        print "****************** codigo monto *********** ", code_amount
        print "****************** valor tabla *********** ", value_isr
        print "****************** aplica por año *********** ", apply_year
        ...
        # Obtiene el numero del periodo que registra
        num_period = self._get_month_period(cr, uid, period_id, context=context)

        # Si aplica por año se consideran los 12 meses del periodo para el calculo
        if apply_year == True:
            num_period = 12

        print "************* numero periodo ********** ", num_period

        # Genera un diccionario con las equivalencias del valor a retornar
        res = {
            'limit_down': 0,
            'limit_top': 1,
            'amount': 2,
        ...
        print "************* valor de la tabla ********** ", res.get(value_isr,0)

        # Obtiene el valor del registro sobre el que aplica en la tabla de isr
        cr.execute("""
            select
                limit_down, limit_top, amount, percent
            from
                (
                    select 
                    case when limit_down <= 0.01 then limit_down else limit_down * %s end as limit_down, 
                    (limit_top * %s) as limit_top, (amount * %s) as amount, percent 
                    from account_fiscal_isr
                ) as ISR
            where limit_down <= %s and limit_top >= %s
                """%(num_period,num_period,num_period,code_amount,code_amount))
        amount = 0.0
        for value in cr.fetchall():
            # Obtiene el resultado segun la etiqueta solicitada
            amount = value[res.get(value_isr,0)]
            break
        return amount
    ...
    def _get_value_condition(self, cr, uid, result, child_ids, date, period_id, fiscalyear, target_move='all', context=None):
        ...
        for child in child_ids:
            ...
            if child.if_value == True and period_id:
                ...
                # Calculo para tabla isr
                elif child.type_code == 'code_isr':
                    code_amount = 0.0
                    # Obtiene el monto del codigo fiscal sobre el calculo
                    if values.get(child.reference.id, False):
                        code_amount = values.get(child.reference.id,False)['result']
                    # Obtiene el valor de la tabla isr
                    amount = self.get_value_code_isr(cr, uid, period_id, child.reference.id, code_amount, child.value_isr, apply_year, context=context)
        ...
        for child in code.child_ids:
            ...
            if period_id:
                ...
                if child.type_code == 'view':
                    ...
                    print "************ calcula por periodo deduccion ************ ", child.reference.exclude_deduction
                    # Valida si se aplican deducciones sobre el rubro fiscal
                    if child.reference.exclude_deduction:
                        print "************ calcula por periodo deduccion ************ ", child.reference.exclude_deduction
            ...
            for child in code.child_ids:
                ...
                if code.period_id:
                    ...
                    # Calculo para tabla isr
                        ...
                        code_amount = 0.0
                        # Obtiene el monto del codigo fiscal sobre el calculo
                        if result.get(child.reference.id, False):
                            code_amount = result.get(child.reference.id,False)['result']
                        # Obtiene el valor de la tabla isr
                        amount = self.get_value_code_isr(cr, uid, code.period_id.id, child.reference.id, code_amount, child.value_isr, apply_year, context=context)
                    elif child.type_code == 'code_period':
    ...
    _columns = {
        ...
        'value_isr': fields.selection([
                        ('limit_down','Limite Inferior'),
                        ('limit_top','Limite Superior'),
                        ('amount','Cuota Fija'),
                        ('percent','(%) S/EXC. L.I')], 'Valor Obtenido'),
    }
...
class account_fiscal_isr(osv.Model):
    _name = 'account.fiscal.isr'

    _columns = {
        'name': fields.char('Nombre', size=128),
        'sequence': fields.integer('Orden'),
        'limit_down': fields.float('Limite inferior', digits_compute= dp.get_precision('Account'), required=True),
        'limit_top': fields.float('Limite superior', digits_compute= dp.get_precision('Account'), required=True),
        'amount': fields.float('Cuota fija', digits_compute= dp.get_precision('Account'), required=True),
        'percent': fields.float('(%) S/EXC. L.I', digits_compute= dp.get_precision('Account'), required=True, help="Porcentaje sobre excedente limite inferior"),
    }
    _defaults = {
        'sequence': 5
    }
