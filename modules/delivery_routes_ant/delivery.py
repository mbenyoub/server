...
class delivery_route(osv.osv):
    ...
    def _confirm_route_carrier(self, cr, uid, ids, context=None):
        ...
        for route in self.browse(cr, uid, ids, context=context):
            # Obtiene la tarifa 
            #for line in route.line_ids:
            #    pricelist_id = line.sale_order_id.pricelist_id.id or False
            #    currency = line.invoice_id.currency_id or False
            #    quantity += line.qty
            ...
            values = {
                ...
                'origin': route.name,
                #'company_id': route.user_id.company_id.id,
            ...
            line_obj.create(cr, uid, details, context=context)
    ...
    def action_prepare_move_stock(self, cr, uid, ids, context=None):
        ...
        line_out_obj = self.pool.get('delivery_route_line')
        if context is None:
            context={}
        #res = {}
        lines = []
        ...
        for move in self.browse(cr, uid, ids, context=context):
            if move.is_carrier:
                is_carrier = move.is_carrier
            else:
                is_carrier = False
            carrier_id = move.carrier_id or False
        
        print "***********IS_CARRIER**********: ", is_carrier
        
        if is_carrier is False:
            ...
            print "*******PREPARANDO EMBARQUE PARA TRANSPORTISTA********"
             ...
             # Crea registro provisional para ejecucion de wizard
            res_id = self.pool.get('delivery.route.out').create(cr, uid, {'route_id': route_id},
                context=context)
            ...
            for move in move_obj.browse(cr, uid, move_ids, context=context):
                ...
                vals1 = {
                    ...
                    'product_uom': move.product_uom.id or False,
                    'location_id': move.location_id.id or False,
                    'location_dest_id': move.carrier_id.property_stock_supplier.id or False,
                    'state': move.state,
                    'route_id': route_id,
                    'move_id': move.id,
                    'wizard_id': res_id,
                ...
                print "********VALS*********: ", vals
                line_out_id = line_out_obj.create(cr, uid, vals, context=context)
    ...
    def update_stock(self, cr, uid, ids, context=None):
        """
            Actualiza en la ruta la informacion del embarque
        """
        res_id = ids[0]
    ...
    def action_return(self, cr, uid, ids, context=None):
        ...
        # Registra el log sobre la actualizacion sobre la transicion de estado
        self.pool.get('delivery.route.log').add_log_route(cr, uid, ids, state='return', context=context)
        return True
