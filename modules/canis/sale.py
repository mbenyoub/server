...
class sale_order(osv.Model):
    ...
    def _compute_mix(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for value in cr.fetchall():
                ...
                weight_order = float(value[2] or 0.0)
                break
    ...
    def _search_expired_documents(self, cr, uid, partner_id, context=None):
        """
            Metodo para buscar facturas expiradas y que no se han pagado, regresara un valor
            booleano ('True') en caso de encontrarlo
        """
        expired = 0
        invoice_obj = self.pool.get('account.invoice')
        print "******PARTNER_ID ****: ", partner_id
        # Realizando la consulta para obtener la fecha de vencimiento de cada una de las facturas
        # del cliente
        cr.execute("""
                    SELECT
                        id,
        ...
        for invoice in cr.fetchall():
            ...
            # print "****DATE SYSTEM ***: ", time.strftime("%Y-%m-%d")
            
            if invoice[1] <= time.strftime("%Y-%m-%d"):
                print "****DOCUMENTO VENCIDO*****"
                state = invoice_obj.browse(cr, uid, invoice[0], context=context)['state']
                if state in ['draft', 'open']:
                    expired = expired + 1
            else:
                print "*****DOCUMENTO NO VENCIDO******"
        
        print "***EXPIRED***: ", expired
        if expired > 0:
            print "****TIENE DOCUMENTOS EXPIRADOS******"
            return True
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            for line in order.order_line:
                ...
                if line.price_unit < line.price_unit_original:
                    print "***SEA DISMINIUIDO EL PRECIO ORIGINAL****"
                    res =  False
                ...
                if line.product_id:
                    print "****OBTENIENDO EL DESCUENTO APLICADO SOBRE EL PRODUCTO*****"
            ...
            # Buscando documentos expirados con adeudos del cliente
            expired = self._search_expired_documents(cr, uid, order.partner_id.id or False,
                context=context)
            print "***EXPIRED****: ", expired
            # Valida que el cliente tenga credito disponible para que le generen la factura
            if (order.credit_available < order.amount_total) or (expired == True):
                print "****CLIENTE CON CREDITO PARA GENERAR FACTURA****"
                # Valida que el mentodo de pago de la factura no sea por pago inmediato
                payment_days = 0.0
                # Obtiene la cantidad de dias sobre el credito
                if order.payment_term:
                    print "***OBTENIENDO DIAS SOBRE EL CREDITO****"
                ...
                if payment_days > 0.0:
                    print "****CUENTA CON PLAZO DE PAGO*******"
                    res = False
                    credit = 0.0
                    if order.credit_available:
                        print "*****CLIENTE CON CREDITO*****"
                        credit = order.credit_available
                    msgs.append("El cliente no cuenta con el credito disponible (Credito disponible: %s) o tiene documentos vencidos."%(credit,))
                    pending = "credit"
            
            # ----MODIFICACION 07-04-2015----
            # Agregacion de validacion de monto de venta sea mayor a precio minimo
            if order.pricelist_id.limit_min_sale < order.amount_total:
                print "***PRECIO MINIMO SUPERADO*****"
                # Valida que res sea verdadero y si es asi es que se hizo alguna modificacion
                if res:
                    print "****SE REALIZO UNA MODIFICACION****"
                    return self.action_button_confirm(cr, uid, ids, context=context)
                else:
                    print "*******NO SE REALIZO ALGUNA MODIFICACION****"
                    # Pasa a por autorizar si no se cumplieron todas las validaciones
                    if pending == "discount":
                        print "****PASANDO POR AUTORIZACION DE DESCUENTO*****"
                        self.write(cr, uid, ids, {'state': 'pending_discount'}, context=context)
                    else:
                        print "****PASANDO POR AUTORIZACION DE CREDITO*****"
                ...
                print "****VENTA NO SUPERO EL PRECIO MINIMO*****"
                raise osv.except_osv(_('Error!'), _('La venta no ha superado el precio minimo %s')%(order.pricelist_id.limit_min_sale,))
