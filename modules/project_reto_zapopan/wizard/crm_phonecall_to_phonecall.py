...
class project_phonecall2phonecall(osv.osv_memory):
    _name = 'project.phonecall2phonecall'
    ...
    _columns = {
        ...
        'action': fields.selection([('schedule','Planificar llamada'), ('log','Registrar llamada')], 'Accion', required=True),
    ...
    def redirect_project_phonecall_view(self, cr, uid, phonecall_id, context=None):
        model_data = self.pool.get('ir.model.data')
        # Select the view
        tree_view = model_data.get_object_reference(cr, uid, 'project_reto_zapopan', 'project_crm_case_phone_tree_view')
        form_view = model_data.get_object_reference(cr, uid, 'project_reto_zapopan', 'project_crm_case_phone_form_view')
        search_view = model_data.get_object_reference(cr, uid, 'project_reto_zapopan', 'view_project_crm_case_phonecalls_filter')
    ...
    def action_log_call(self, cr, uid, ids, context=None):
        ...
        return self.redirect_project_phonecall_view(cr, uid, phonecall_id, context=context)
    ...
    def default_get(self, cr, uid, fields, context=None):
        ...
        """
        res = super(project_phonecall2phonecall, self).default_get(cr, uid, fields, context=context)
