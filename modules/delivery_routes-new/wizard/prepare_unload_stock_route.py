...
class prepare_unload_stock_route_wizard(osv.osv_memory):
    _name = 'prepare.unload.stock.route'
    ...
    _columns = {
        ...
        'line_ids': fields.one2many('prepare.unload.stock.route.line', 'wizard_id', 'Embarque'),
    }
    
prepare_unload_stock_route_wizard()

class prepare_unload_stock_route_line_wizard(osv.osv_memory):
    """
        Movimientos de stock
    """
    _name = "prepare.unload.stock.route.line"
    
    def action_done(self, cr, uid, ids, context=None):
        """
            Prepara la descarga de vehiculo
        """
        route_id = False
        res_id = False
        move_obj = self.pool.get('stock.move')
        ...
        for line in self.browse(cr, uid, ids, context=context):
            if line.move_id:
                ...
                if line.product_qty > line.virtual_available:
                    raise osv.except_osv(_('Error'), _("No hay producto disponible para descargar este producto de la ruta la Ruta. (Producto: %s)"%(line.name,)))
                # Pone el producto como descargado de la camioneta
        ...
        return self.pool.get('delivery.route').update_unload_stock(cr, uid, [route_id], context=context)
    ...
    _columns = {
        ...
        'wizard_id': fields.many2one('prepare.unload.stock.route', 'Wizard', ondelete="cascade"),
...
prepare_unload_stock_route_line_wizard()