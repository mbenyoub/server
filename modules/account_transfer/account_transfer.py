...
class account_transfer(osv.osv):
    ...
    _columns = {
        ...
        'type': fields.selection(_get_type,'Tipo', required=True, readonly=True, states={'draft':[('readonly',False)]}),
        ...
        'origin': fields.char('Origen', size=128, readonly=True, states={'draft':[('readonly',False)]},help="Ducumento de origen o referencia"),
        'account_analytic_id': fields.many2one('account.analytic.account', 'Cuenta Analitica', readonly=True, states={'draft':[('readonly',False)]}),
        'voucher_ids': fields.one2many('account.voucher','transfer_id', string='Pagos', readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)]}),
        'src_journal_id': fields.many2one('account.journal','Banco Origen',required=True, domain=[('type','in',['cash','bank'])], select=True, readonly=True, states={'draft':[('readonly',False)]}),
        'src_partner_id': fields.many2one('res.partner','Contacto Origen', select=True),
        'src_balance': fields.function(_balance, digits_compute=dp.get_precision('Account'), string='Saldo Banco origen', type='float', readonly=True, multi='balance', help="Saldo actual sobre la cuenta asignada al diario"),
        'src_amount': fields.float('Monto a Transferir',required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'src_have_partner': fields.related('src_journal_id','have_partner',type='boolean',string='Contacto Requerido',readonly=True),
        'dst_journal_id': fields.many2one('account.journal','Banco Destino',required=True, domain=[('type','in',['cash','bank'])], select=True, readonly=True, states={'draft':[('readonly',False)]}),
        'dst_partner_id': fields.many2one('res.partner','Contacto Destino', select=True),
        'dst_balance': fields.function(_balance, digits_compute=dp.get_precision('Account'), string='Saldo Banco destino', type='float', readonly=True, multi='balance', help="Saldo actual sobre la cuenta asignada al diario"),
        'dst_amount': fields.float('Monto transferido',required=True, readonly=True, states={'draft':[('readonly',False)]}),
        'dst_have_partner': fields.related('dst_journal_id','have_partner',type='boolean',string='Contacto Requerido',readonly=True),
        'exchange_rate': fields.float('Tipo de cambio', digits_compute=dp.get_precision('Exchange'), readonly=True, states={'draft':[('readonly',False)]}),
        'exchange': fields.function(_balance, string='Cambia Moneda', type='boolean', readonly=True, multi='balance'),
        'exchange_inv': fields.function(_balance, string='1 / Cambia moneda', type='float', digits_compute=dp.get_precision('Exchange'), readonly=True, multi='balance'),
        'adjust_move': fields.many2one('account.move','Ajustar movimiento', readonly=True, help="Ajuste del movimiento usualmente aplicado por el cambio de moneda"),
        'state': fields.selection(STATE_SELECTION, string='Estado', track_visibility='onchange', readonly=True),
    ...
    _defaults = {
        ...
        'exchange_inv': 1.0,
        'state': 'draft',
        'src_partner_id': _get_default_partner_company,
        'dst_partner_id': _get_default_partner_company,
    }
    
    _sql_constraints = [('name_unique','unique(company_id,name)',_('El numero debe ser unico!'))]
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for trans in self.browse(cr, uid, ids, context=context):
            if trans.state not in ('draft'):
                raise osv.except_osv(_('User Error!'),_('No puedes eliminar un, traspaso que no este en estado borrador: "%s"') % trans.name)
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for trans in self.browse(cr, uid, ids, context=context):
            ...
            dval['line_ids'][0][2]['account_id'] = trans.dst_journal_id.default_debit_account_id.id
            ...
            period_id = self.get_period_on_date(cr, uid, trans.date, context=context)
            if period_id:
                self.write(cr, uid, [trans.id], {'period_id': period_id},context=context)
