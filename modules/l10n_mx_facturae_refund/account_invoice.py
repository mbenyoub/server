...
class account_invoice(osv.osv):
    ...
    def _create_refund_debit_note(self, cr, uid, invoice, mode='none', context=None):
        ...
        debit_journal = invoice.journal_id.journal_debit_id
        # Valida que exista
        if not debit_journal:
            raise osv.except_osv(_('Error!'),_("Diario automatico para nota de debito, no configurado. Configure el diario sobre "))
        if debit_journal.type not in ('sale_debit','purchase_debit'):
            raise osv.except_osv(_('Error!'),_("Configurar el diario de nota de debito con: debito de compra o debito de venta"))
        ...
        invoice_data.update({
            ...
            'debit_note': True,
            'filter_refund': mode,
            'journal_id': debit_journal.id or False
        })
