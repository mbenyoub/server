...
class crm_kober_ws_control(osv.Model):
    ...
    def ws_get_cte_create(self, cr, uid, top, context=None):
        ...
        #print "**************** result get_cte_create *********** ", result
        ...
        for row in tree.find('Rows'):
            ...
            for field in row:
                ...
                if field.tag == 'Agente':
                    ...
                    continue
                elif field.tag == 'descripcion1':
                    # Obtiene el cliente que recomendo
                    partner_ids = partner_obj.search(cr, uid, [('ws_id', '=', field.text),])
                    if not partner_ids:
                        #~ Si no esta el cliente registrado deja el campo vacio
                    ...
                    if not state_ids:
                        ...
                        if pais == 'Mexico' or pais == 'México' or 'MEXICO':
                            ...
                            country_ids = country_obj.search(cr, uid, ['|',('name', '=', pais),('code','=',pais[:2])])
                            if not country_ids:
    ...
    def ws_get_cte_update(self, cr, uid, top, context=None):
        ...
        for row in tree.find('Rows'):
            ...
            for field in row:
                ...
                if not field.text: 
                   continue
                print "*********** field tag *********** ", field.tag, " - ", field.text, " - ", field_name[field.tag]
                # Recorre los campos del cliente
                if field.tag == 'Agente':
                    # Obtiene el id del vendedor en openerp, si no lo encuentra deja el campo vacio
                    user_ids = user_obj.search(cr, uid, [('code', '=', field.text),])
    ...
    def ws_put_cte(self, cr, uid, ids, context=None):
        ...
        for partner in partner_obj.browse(cr, uid, ids, context=context):
            ...
            vals = [
                ...
                "descripcion1 = '" + self.remove_tildes(partner.parent_id2.client) + "'" if partner.parent_id2 else "descripcion1 = ''",
            ]
            vals_string = u''
    ...
    def ws_post_cte(self, cr, uid, ids, context=None):
        ...
        for partner in partner_obj.browse(cr, uid, ids, context=context):
            ...
            vals = [
                ...
                "'" + self.remove_tildes(partner.parent_id2.client) + "'" if partner.parent_id2 else "''",
            ]
            vals_string = u''
            vals_string = ",".join(vals)
    ...
    def ws_get_ctecto_create(self, cr, uid, top, context=None):
        ...
        for row in tree.find('Rows'):
            ...
            # Recorre los campos a agregar
            for field in row:
                print "************* registro ************** ", field.text, ' - ', field_name[field.tag]
                # Valida que el field tenga valor
                if not field.text: 
                   continue
            ...
            # Agrega el campo nombre si no lo trae
