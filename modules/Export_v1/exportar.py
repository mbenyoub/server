...
exportar()
        ...
        #raise osv.except_osv('carpeta', carpeta)
        #return {
        #    'type': 'ir.actions.report.xml',
        #}
        #libro.save("tmp/prueba.xls")
        #xls = open('tmp/resultsfile.csv', 'w')
        #xls.close()
        
        # with open('tmp/excel.xls', 'w')  as f:
        #     writer = csv.writer(f, delimiter=',')
        #     cr.copy_expert(outputquery, f)
...
#        f.write("hi")
#        f.close()
#        os.rename(f.name, r'/tmp/foo.txt')
...
#        streaming.csv_generator(outputquery)
#        streaming.csv_stream_response(response, outputquery, file_name="xxxx.csv")
        #return output
        #raise osv.except_osv(pwd.getpwuid(os.getuid()).pw_gecos, os.path.expanduser('~'))
    
        # if context is None:
        #     context = {}
        # res = self.exportar(cr, uid, ids, context=context)
        # data = {}
        # data['form'] = outputquery#self.pool.get('account.annual.balance').read(cr, uid, ids, ['fiscalyear', 'account_code'], context=context)[0]
        # #for field in ['fiscalyear', 'account_code']:
