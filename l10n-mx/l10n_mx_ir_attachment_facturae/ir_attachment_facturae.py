...
class ir_attachment_facturae_mx(osv.Model):
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        if type == 'cfdi32':
            # Agrega validacion de informacion de direccion de factura
            invoice_obj.validate_invoice_partner(cr, uid, invoice.id, context=context)
    ...
    def action_printable(self, cr, uid, ids, context={}):
        ...
        # Cambia formato a string
        invoice.fname_invoice = str(invoice.fname_invoice)
        ...
        for attachment in self.pool.get('ir.attachment').browse(cr, uid, adjuntos, context=context):
            #print"******************* ver si es por el ciclo for ******************** "
            aids = attachment.id
            
            #print"******************** attachment obj *************** ", attachment
            #print"******************** attachment ids *************** ", attachment.id
            #print"******************** attachment *************** ", invoice.fname_invoice + '.pdf'
            
            name = invoice.fname_invoice + '.pdf'
            self.pool.get('ir.attachment').write(cr, uid, [attachment.id], {'name': name}, context={})
            
            at = self.pool.get('ir.attachment').browse(cr, uid, attachment.id, context)
            
            #print"************** continua attachment *********** ", at
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        ...
        msj = self.browse(cr, uid, ids)[0].msj
        if type == 'cfdi32':
            #print"******** eliminar factura ************ ", invoice.id
