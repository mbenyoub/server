...
class account_invoice(osv.Model):
    ...
    def _get_facturae_invoice_dict_data(self, cr, uid, ids, context={}):
        ...
        for data in datas:
            if type_inv == 'cfdi32':
                print "************* Comprobante ************ ", data['Comprobante']
                ...
                print "************* impuestos retenidos ************ ", comprobante['Impuestos'].get('Retenciones',False)
                # Valida si el comprobante tiene impuestos retenidos
                if comprobante['Impuestos'].get('Retenciones',False):
                    totalImpuestosRetenidos = comprobante['Impuestos']['totalImpuestosRetenidos']
                    dict_impuestos.update({'totalImpuestosRetenidos':
                                       totalImpuestosRetenidos, 'cfdi:Retenciones': []})
                    print "********** dict impuestos ************ ", dict_impuestos
                    for retencion in comprobante['Impuestos']['Retenciones']:
                        dict_impuestos['cfdi:Retenciones'].append(dict({'cfdi:Retencion': retencion['Retencion']}))
