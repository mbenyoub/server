...
class invoice_facturae_html(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        ...
        self.localcontext.update({
            ...
            'get_data_partner_delivery': self._get_data_partner_delivery,
            ...
            'check_pay_method': self._check_pay_method
        })
        self.taxes = []
    ...
    def _get_data_partner_delivery(self, partner_id):
        address_invoice = ''
        partner_obj = self.pool.get('res.partner')
        res = {}
        address_invoice = partner_obj.browse(self.cr, self.uid, partner_id.id)
        if address_invoice.use_parent_address and address_invoice.parent_id:
            id_parent = address_invoice.parent_id.id
        ...
        if address_invoice:
            ...
            raise openerp.exceptions.Warning(
                'Direccion de envio sobre factura invalido')
        return res
    ...
    def _get_data_partner(self, partner_id):
        ...
        if address_invoice:
            ...
            # print "Customer Address Not Invoice Type"
            raise openerp.exceptions.Warning(
                'Direccion de cliente de factura invalido')
        return res
    ...
    def _check_pay_method(self, invoice_id):
        """
            Revisa si el metodo de pago es transferencia electronica
        """
        pool = pooler.get_pool(self.cr.dbname)
        invoice_obj = pool.get('account.invoice')
        ...
        pay_id = False
        try:
            payment = data_obj.get_object(self.cr, self.uid, 'l10n_mx_payment_method', 'pay_method_transferencia_electronica')
        ...
        if invoice.pay_method_id:
            print "******************* pay_method_id ***************** ", invoice.pay_method_id
            print "******************* pay_id ***************** ", pay_id
            if invoice.pay_method_id.id == pay_id:
                res = True
            print "************* pay method **************** ", res
        return res
