...
class account_invoice(osv.Model):
    ...
    _defaults = {
        'invoice_datetime': lambda self,cr,uid,context=None: fields.date.context_today(self,cr,uid,context) + (datetime.datetime.today()+datetime.timedelta(minutes=1)).strftime(' %H:%M:%S')#(datetime.datetime.today()+datetime.timedelta(minutes=1)).strftime('%Y-%m-%d %H:%M:%S'),#datetime.datetime.today().strftime('%Y-%m-%d %H:%M:%S'),#time.strftime('%Y-%m-%d %H:%M:%S'),
    ...
    def assigned_datetime(self, cr, uid, values, context=None):
        ...
        if 'date_invoice' in values:
            if values['date_invoice']:
                ...
                time_invoice = datetime.time(now.hour, datetime.datetime.now().minute+1, now.second)
        ...
        if 'invoice_datetime' in values:
            if values['invoice_datetime']:
                ...
                fecha = datetime.datetime.strptime(values['invoice_datetime'], '%Y-%m-%d %H:%M:%S')
                res['invoice_datetime'] = fecha+datetime.timedelta(minutes=1)

                
        #res['invoice_datetime'] = time.strftime('%Y-%m-%d %H:%M:%S')   
        #res['date_invoice'] = time.strftime('%Y-%m-%d %H:%M:%S')
        return res
