...
# Cambiar el error
msg2 = "Contact you administrator &/or to info@akkadian.com.mx"
...
class account_invoice(osv.osv):
    ...
    _columns = {
        ...
        'rate': fields.float('Type of Change', readonly=True, help='Rate used in the date of invoice'),
    ...
    def write_cfd_data(self, cr, uid, ids, cfd_datas, context={}):
        ...
        if True:
            ...
            serie = cfd_data.get('Comprobante', {}).get('serie', '')
...
# TODO: agregar esta funcionalidad con openssl
    ...
    def validate_invoice_partner(self, cr, uid, id, context=None):
        """
            Validacion sobre informacion de cliente a facturar
        """
        invoice = self.browse(cr, uid, id, context=context)
        # Concacto donde se valida la informacion de la empresa
        partner = invoice.partner_id
        if invoice.partner_id.commercial_partner_id:
            partner = invoice.partner_id.commercial_partner_id
        
        # Valida el rfc del cliente
        if not partner.rfc:
            raise osv.except_osv(_('Error Validacion!'),_("El cliente '%s' no tiene un RFC Valido!")%(partner.name,))
        # Valida la direccion del cliente
        if not partner.street:
            raise osv.except_osv(_('Error Validacion!'),_("El cliente '%s' no tiene una calle Valida!")%(partner.name,))
        if not partner.city:
            raise osv.except_osv(_('Error Validacion!'),_("El cliente '%s' no tiene una ciudad Valida!")%(partner.name,))
        if not partner.state_id:
            raise osv.except_osv(_('Error Validacion!'),_("El cliente '%s' no tiene un estado Valido!")%(partner.name,))
        if not partner.country_id:
            raise osv.except_osv(_('Error Validacion!'),_("El cliente '%s' no tiene un pais Valido!")%(partner.name,))
        
        # Valida si hay una direccion de envio sobre el cliente
        if invoice.part_sh_id:
            if invoice.partner_id.id != invoice.part_sh_id.id:
                partner = invoice.part_sh_id
                if partner.use_parent_address:
                    partner = partner.parent_id
                # Valida informacion de direccion de envio
                if not partner.street:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene una calle Valida!")%(partner.name,))
                if not partner.street2:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene una colonia Valida!")%(partner.name,))
                if not partner.city:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene una ciudad Valida!")%(partner.name,))
                if not partner.state_id:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene un estado Valido!")%(partner.name,))
                if not partner.country_id:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene un pais Valido!")%(partner.name,))
                if not partner.zip:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene un codigo postal Valido!")%(partner.name,))
                if not partner.l10n_mx_street3:
                    raise osv.except_osv(_('Error Validacion!'),_("La direccion de envio del cliente '%s' no tiene un numero de direccion Valido!")%(partner.name,))
        return True
    ...
    def _get_facturae_invoice_dict_data(self, cr, uid, ids, context={}):
        ...
        for invoice in invoices:
            ...
            invoice_data_parent['Comprobante'].update({
                ...
                'formaDePago': u'Pago en una sola exhibicion',
                'noCertificado': '@',
                'sello': '@',
                'certificado': '@',
...
# modulo TaxIncluded
                ...
                unidad = line.uos_id and line.uos_id.name or 'No aplica'
                if unidad:
            ...
            for line_tax_id in invoice.tax_line:
                ...
                print "**************** impuestos monto *********** ", line_tax_id.amount, " -- abs ", line_tax_id_amount
                ...
                print "******* Agrega impuesto a xml ************** ", impuesto_dict
                impuesto_list.append(impuesto_dict)

            print "*************** total impuestos retenidos ************ ", totalImpuestosRetenidos
            invoice_data['Impuestos'].update({
