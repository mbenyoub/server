...
class ir_sequence_approval(osv.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            if record.type == 'cfdi32':
                name = "Sec. aprobacion CFDI %s"%(record.serie)
            else:
                name = "Sec. aprobacion CFD %s"%(record.approval_number)
    ...
    _columns = {
        ...
        'approval_number': fields.char(u'Approval Number', size=64,
            required=False, help='Name of the type of Electronic Invoice to \
            configure'),
        'serie': fields.char(u'Serie of Folios', size=12, required=False,
            help="With which report to SAT, example. FA (for Invoices), NC \
            (For Invoice Refund)"),
        'approval_year': fields.char('Year Approval', size=32, required=False,
        ...
        'number_end': fields.integer(u'Until', required=False,
    ...
    def _check_numbers_range(self, cr, uid, ids, context=None):
        ...
        # Revisa que la secuencia sea cfd para validar el numbero de aprobacion
        if approval.type == 'cfd22':
...
class ir_sequence(osv.Model):
    ...
    def _get_current_approval(self, cr, uid, ids, field_names=None, arg=False,
        ...
        for sequence in self.browse(cr, uid, ids, context=context):
            # Busca secuencias de numero de aprobacion por cfdi
            ...
            if not approval_id:
                # Busca secuencias de aprobacion por cfd
