...
class account_invoice(osv.osv):
    def divisas(self, cr, uid, dolares, context=None):
    #Apuntamos a la pagina de la cual queremos obtener la informacion
        response = urllib2.urlopen('http://www.dof.gob.mx/indicadores.xml')

    #Declaramos una variable para guardar el resultado de la pagina consultada
        html = response.read()

    # Cerramos la conexion una vez obtenido el resultado
        response.close() 
    ...
    #Declaramos las variables que vamos a utilizar
        ...
        while i < len(html):
            i += 1
            if html.find("DOLAR") >= 0:
                for innerline in range(0, 15):
                    try:
                        #raise osv.except_osv('res', int(html[823+30+con]))
                        if int(html[823+30+con]) >= 0:
                            #raise osv.except_osv('res', html[823+30+con])
                            dolar = dolar + html[823+30+con]
                            con = con + 1
                    except ValueError:
                        if html[823+30+con] == '.':
                            dolar = dolar + html[823+30+con]
        ...
        #agregamos un par de ceros para completar los 6 decimales de la divisa
        #raise osv.except_osv('Dolar: ', dolar)
        dolar = dolar + "00" 
        if float(dolar)<1:
            cr.execute("select rate from res_currency_rate where currency_id = 3 order by id desc limit 1")
            usd = cr.fetchall()
            for moneda in usd:
                dolar = moneda[0]
        else:
        ...
        #realizamos la consulta para saber cual fue el ultimo id con el comando execute
        cr.execute("SELECT id FROM RES_CURRENCY_RATE order by id desc limit 1")
        idres = cr.fetchall()

        #realizamos la segunda consulta para saber cuantos registros se han realizado por dia
        cr.execute("SELECT * FROM RES_CURRENCY_RATE where date(create_date) = date(current_timestamp)") 
        row = cr.rowcount
        ...
        #entramos en un bucle, en el cual obtendremos el resultado del id
        for res in idres:
            id = res[0]
        ...
        for today in hoy:
            ahora = today[0]    
        #aumentamos el id en 1 para posteriormente ingresarlo en la base de datos  
        id = id + 1
  
        #utilizamos una condicional para comprobar que no se haya metido el dato aun, en caso de que ya exista el registro por dia de la divisa
        #hara caso omiso y no ingresara ningun registro si no hasta el siguiente dia
        if row==0:
            divisa = self.pool.get('res.currency.rate')
            dolares = divisa.create(cr, uid, {
            'id': id,
            'create_uid': 1,
            'create_date': actual,
            'write_date': actual,
        ...
        #cr.execute("INSERT INTO res_currency_rate (id, create_uid, create_date, write_date, currency_id, rate, name) values (%s, 1, current_timestamp, current_timestamp, 3, %s, current_date)" % (id, dolar))            
    ...
    _columns = {
        ...
        #'uuid': fields.char('UUID', size=64, readonly=True)
        #'date_arrive': fields.date('Arrive date', select=True, help="Keep empty to use the current date"),
        #'pedimento': fields.char('Pedimento', size=64),
    }
    _defaults = {
    ...
    def remision_validate(self, cr, uid, ids, context=None):
        self.write(cr, uid, ids,{'state':'open'}, context=context)
        return True

    def line_get_convert(self, uid, x, part, date, context=None):
        return {
    ...
    def pay_and_reconcile(self, cr, uid, ids, pay_amount, pay_account_id, period_id, pay_journal_id, writeoff_acc_id, writeoff_period_id, writeoff_journal_id, context=None, name=''):
        ...
        if 'amount_currency' in context and context['amount_currency'] and 'currency_id' in context and context['currency_id']:
            ...
            raise osv.except_osv(amount_currency, context['amount_currency'])
