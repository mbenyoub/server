...
class account_move_line(osv.osv):
    ...
    def _check_currency_and_amount(self, cr, uid, ids, context=None):#Correccion 
        #for l in self.browse(cr, uid, ids, context=context):
            #if (l.amount_currency and not l.currency_id):
            #    return False
        return True

    def _check_currency_amount(self, cr, uid, ids, context=None):#Correccion validacion debito y credito moneda diferente a la moneda base
        for l in self.browse(cr, uid, ids, context=context):
            ...
            if l.amount_residual > 0:
                amount_currency = l.amount_residual
        #           raise osv.except_osv('id '+str(l.amount_residual)+' amount_currency '+str(l.amount_currency) , 'debit '+str(l.debit)+' credit '+str(l.credit))
        #           raise osv.except_osv('res', amount_currency)
        return True
           ...
           (account.currency_id and (not currency_obj.is_zero(cr, uid, account.currency_id, currency))):
            ...
            writeoff_lines = [
                (0, 0, {
                    ...
                    'amount_currency': 0.0#amount_currency_writeoff and -1 * amount_currency_writeoff or (account.currency_id.id and -1 * currency or 0.0)
                }),
                (0, 0, {
                    'name': libelle,
                    'debit': debit,
                    ...
                    'partner_id': partner_id,
                    'currency_id': cur_id or (account.currency_id.id or False),
                    'amount_currency': 0.0#amount_currency_writeoff and amount_currency_writeoff or (account.currency_id.id and currency or 0.0)
                })
            ]
    ...
    def _datos(self, cr, uid, ids, context=None):
    ...
    def create(self, cr, uid, vals, context=None, check=True):
        ...
        #raise osv.except_osv('vals', self.amount)
        #if 'reference' not in vals:
        #    total = vals['debit']
        #Cambiar el valor del importe en dolares
        # if 'debit' in vals=
        #     if vals['debit'] == 0.16:
        #         raise osv.except_osv('hi', vals
        #res.append(vals)
        #if len(res) >= 2:
        #raise osv.except_osv(vals,vals['credit'])

        #if 'IVA' in vals['name']:
        #    raise osv.except_osv('vals',vals)
        ...
        if 'currency_id' in vals:
            ...
            #if '/' in vals['name']:
            #    total = vals['debit']
            if vals['currency_id'] != False and 'reference' in vals:#and 'tax_amount' in vals :# Correccion para dar el valor al importe en moneda extranjera y cambiar estatus factura
                
            #raise osv.except_osv('res', vals)
                move_ids = move_obj.search(cr, uid, [('id','=',vals['move_id'])])
                for move in move_obj.browse(cr, uid, move_ids):
                    ref = move.reference.id

                vou_ids = vou_obj.search(cr, uid, [('id','=',move.reference.id)])
                for voucher in vou_obj.browse(cr, uid, vou_ids):
                    amount = voucher.amount

                #if vals['debit']> 0:
                    #vals['amount_currency'] = vals['debit']
                
                
                #vals['amount_currency'] = 0.0#(amount * 0.16)
                #raise osv.except_osv(total, vals)
                if vals['debit']> 0 and 'IVA' in vals['name']:
                    
                    rate_ids = rate_obj.search(cr, uid, [('currency_id','=',vals['currency_id'])])
                    for rate_for in rate_obj.browse(cr, uid, rate_ids):
                        rate = rate_for.rate
                        break
                    #tax_ids = 
                    vals['debit'] = ((amount / 1.16) * 0.16) / rate#(amount / 1.16 * 0.16) / rate
                    vals['amount_currency'] = amount#vals['debit']
                    vals['base'] = (amount / 1.16) / rate
                    #raise osv.except_osv(rate, vals)
                    #tot_ids = self.browse(cr, uid, [('move_id','=',vals['move_id'])])
                    #for total in self.browse(cr, uid):    
                #    raise osv.except_osv(total, vals)
                if vals['credit']> 0 and 'IVA' in vals['name']:
                    #raise osv.except_osv('vals', vals)
                    rate_ids = rate_obj.search(cr, uid, [('currency_id','=',vals['currency_id'])])
                    for rate_for in rate_obj.browse(cr, uid, rate_ids):
                        rate = rate_for.rate
                        break
                    vals['credit'] = ((amount / 1.16) * 0.16) / rate#(amount / 1.16 * 0.16) / rate
                    vals['amount_currency'] = amount#vals['debit']
                    vals['base'] = (amount / 1.16) / rate
                    #vals['credit'] = (amount * 0.16) / rate#(amount / 1.16 * 0.16) / rate#vals['credit']/rate
                invoice_num = str(vals['reference']).split(',')[1] 
                cr.execute("""update account_invoice set state = 'paid' where id = %s and residual = %s"""%(int(invoice_num),vals['debit']))
                cr.commit()
            #for inv in inv_act:
            #    raise osv.except_osv(inv_act, inv)
        ...
        if vals.get('account_tax_id', False):
            ...
            for tax in tax_obj.compute_all(cr, uid, [tax_id], total, 1.00, force_excluded=False).get('taxes'):
                ...
                raise osv.except_osv('res', data['tax_amount'])
                if data['tax_code_id']:
                    self.create(cr, uid, data, context)
